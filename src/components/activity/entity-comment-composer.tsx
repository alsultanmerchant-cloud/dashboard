...
// Entity-agnostic comment composer (contracts & clients).
// Adapted from tasks/comment-composer.tsx: keeps the floating composer, the
// message/note mode tabs, @mention detection + dropdown, and the browser
// storage attachment flow. The task-only comment-kind chips and stage logic
// are dropped — this composer just posts a message (client-facing) or a note
// (internal) against a contract/client entity.
...
import { addEntityCommentAction } from "@/lib/actions/entity-comments";
...
export function EntityCommentComposer({
  ...
  function submit() {
    ...
    start(async () => {
        ...
        try {
          ...
          const uploads = await Promise.all(
            files.map(async ({ file }) => {
              ...
              const path = `entity-comments/${entityType}/${entityId}/${Date.now()}-${Math.random()
                .toString(36)
                .slice(2, 8)}-${safeName}`;
      ...
      const res = await addEntityCommentAction({
        entityType,
        entityId,
      ...
      toast.success(
        res.mentionsResolved > 0
          ? t("toasts.postedWithMentions", { count: res.mentionsResolved })
          : t("toasts.posted"),
      );
      setBody("");
      setFiles([]);
