...
- عند ذكر اتجاه (تحسّن/تراجع) استند حصريًا إلى حقول trend/prev المعطاة.
- **تمييز إلزامي في المؤشرات**: \`periodValue\` هو رقم الفترة (وهو الرقم الذي تقارنه \`trend\`)، بينما \`liveValue\` لقطة اليوم فقط. لا تخلط بينهما أبدًا، وحين تذكر رقمًا وضّح أيّهما تقصد (مثلاً «خلال الفترة» مقابل «حتى اليوم»). عند الحديث عن تغيّر أو اتجاه استخدم \`periodValue\` حصرًا.${dataQualityBlock}`;
}
