// Mirror Odoo ir.attachment metadata for project.task and project.project
// records. Binaries stay in Odoo; the dashboard renders an "open original"
// link via Odoo /web/content.

import { supabaseAdmin } from "@/lib/supabase/admin";
import type { OdooClient } from "@/lib/odoo/client";

export type AttachmentsSyncResult = {
...
const FIELDS = ["id", "name", "res_model", "res_id", "mimetype", "file_size", "create_date", "type", "url"];

export async function syncAttachments(
  odoo: OdooClient,
  orgSlug: string,
  opts: { log?: boolean } = {},
): Promise<AttachmentsSyncResult> {
  ...
  async function syncFor(model: "project.task" | "project.project"): Promise<number> {
    ...
    for (let i = 0; i < ids.length; i += CHUNK) {
      ...
      if (error) {
        if (log) console.warn(`[${targetTable}] chunk @${i}: ${error.message}`);
      } else {
  ...
  if (log) console.log(`[attachments] DONE — task=${taskAttachments}, project=${projectAttachments}`);
