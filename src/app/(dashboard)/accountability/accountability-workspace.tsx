  ...
  for (const r of rows) {
    ...
    if (r.overdueOwned > 0) {
      issues.push({
        ...
        title: "ترك مهام على مكتبه بعد مهلة المرحلة",
        why: "هذا خطأ لأن بقاء المهمة في المرحلة الحالية بعد مهلة الـSLA يعطّل الخطوة التالية، خصوصًا عندما تكون هذه المرحلة مملوكة لدوره من قالب المهمة.",
        evidence: `${r.overdueOwned} مهمة معلّقة متأخرة من ${r.openTasks} على مكتبه الآن${nonOverdue === null ? "" : `، ونسبة غير المتأخر ${nonOverdue}%`}.`,
        nextAction: "يراجع قائمة «معلّقة متأخرة» يوميًا، يكتب سبب التعطّل كتحديث واضح، ثم يحرّك المهمة أو يطلب إزالة العائق قبل نهاية يوم العمل.",
        source: "نبض الفريق + ملكية المرحلة + مهلة SLA",
