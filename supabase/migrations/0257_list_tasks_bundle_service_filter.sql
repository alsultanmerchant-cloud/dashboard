-- 0257: give /tasks a real service-line filter (p_service_ids).
--
-- The CEO dashboard's "صحة خطوط الخدمة" rows linked to /tasks?service=<id>, but
-- /tasks never accepted a `service` param and `sf` only whitelists
-- title/tags/assignee/project/stage. The param was silently dropped, so clicking
-- a service landed on the FULL unfiltered board (verified live: 560 of 1,394
-- tasks across mixed services). That violates the drill-down standard — a card
-- must land on exactly the set it counted.
--
-- Filtering has to happen IN the RPC: the bundle is capped at a board LIMIT, so
-- filtering post-fetch would slice an already-truncated global page and report
-- the wrong totals — the same class of bug.
--
-- p_service_ids is an ARRAY because the dashboard merges "Renewal Social Media"
-- into "Social Media" (one row, several service ids); the drill must cover every
-- id the row counted.
--
-- Recreated from 0242 VERBATIM (generated, not retyped) with exactly two edits:
-- the new parameter and its predicate. The old 30-arg signature is dropped first
-- so the added parameter cannot leave an ambiguous overload behind.

drop function if exists public.list_tasks_bundle(p_org_id uuid, p_limit integer, p_offset integer, p_status text[], p_stage text[], p_priority text[], p_project_id uuid, p_overdue boolean, p_due_today boolean, p_behind_schedule boolean, p_ahead_schedule boolean, p_critical_delay boolean, p_progress_buckets text[], p_starred boolean, p_followed_by_user_id uuid, p_assigned_to_employee_id uuid, p_search text, p_date_filters jsonb, p_has_start_date boolean, p_has_end_date boolean, p_no_deadline boolean, p_unassigned boolean, p_over_timesheets boolean, p_near_timesheets boolean, p_archived boolean, p_include_archived boolean, p_search_facets jsonb, p_task_ids uuid[], p_partition_by text, p_partition_limit integer);
...
create or replace function public.list_tasks_bundle(
  ...
  -- Service-line scope. Array (not scalar) because the dashboard's service rows
  -- MERGE a service with its "Renewal X" twin, so one row spans several ids.
...
begin
  ...
  with filtered as (
    ...
    where t.organization_id = p_org_id
      ...
      and (p_service_ids is null or cardinality(p_service_ids) = 0 or t.service_id = any(p_service_ids))
      ...
      and (
        ...
        -- Match the human-readable task code (e.g. PRJ-01826-022) so code
        -- deep-links resolve; the FTS vector never contains it. 0242.
        or (t.task_code is not null and t.task_code ilike '%' || v_search || '%')
        or exists (
...
-- The filter is only useful if it's cheap: tasks.service_id had no index.
create index if not exists tasks_org_service_idx
  on public.tasks (organization_id, service_id)
