-- 0093_list_projects_page_bundle_v3.sql
--
-- Sky Light feedback (projects filter-popup parity with Odoo): extend the
-- bundle RPC with two new boolean filters so the projects search bar can
-- expose "All Categories Archived" and "Timesheets >100%" without splitting
-- into separate round-trips.
--
--   • p_all_categories_archived: project has 0 active categories (the
--     opposite of p_only_with_categories — "only show projects whose linked
--     packages are all archived").
--
--   • p_over_timesheets: project has at least one task where logged hours
--     exceed allocated_time_minutes/60 (Rwasem's "over-budget" filter).
--     A task with allocated_time_minutes null/0 is ignored.
--
-- Postgres won't let `create or replace function` change the parameter list
-- (a new param shifts the signature), so we drop the old signature first.
-- The body is verbatim from 0084 except for the two new filter clauses.
