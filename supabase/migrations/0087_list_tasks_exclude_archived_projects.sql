-- Once the importer starts pulling archived projects + tasks (so chatter on
-- completed engagements is reachable), the global /tasks list and kanban
-- would otherwise be flooded with thousands of old "done" rows. Filter them
-- here at the source: the bundle excludes tasks whose parent project has
-- status='archived', UNLESS the caller passed an explicit p_project_id (in
-- which case they're navigating into that project directly and want to see
-- everything inside it, archived or not).
--
-- Same signature, same JSON shape — drop-in replacement for the v0085 RPC.
...
begin
  ...
  with filtered as (
    ...
    where t.organization_id = p_org_id
      -- Hide archived-project tasks unless the caller asked for that
      -- project specifically. `not exists` instead of a join keeps the
      -- query plan tight (no extra row multiplication on task_assignees).
      and (
