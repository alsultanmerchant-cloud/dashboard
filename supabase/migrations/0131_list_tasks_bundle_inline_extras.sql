-- Inline task_tag_assignments + sequence + external_id into list_tasks_bundle
-- so the kanban/list views need only one round-trip instead of three
-- (RPC + extras SELECT + tags SELECT). Saves 2 RTTs per /tasks render.
