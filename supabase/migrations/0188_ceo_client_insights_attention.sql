-- 0188_ceo_client_insights_attention.sql
--
-- The CEO "Top clients: revenue and experience" panel was ranked purely by
-- revenue, so it surfaced high-revenue clients with nothing wrong and showed
-- "next renewal <far date>" as their risk signal — noise the team didn't want.
-- Re-scope it to an ACTION LIST: only clients that actually need attention, by
-- severity. "Needs attention" = an open AI risk, overdue installments, low
-- satisfaction (at-risk or watch, mirroring the رضا العملاء tiers: negative
-- sentiment or score < 70), or a renewal due soon (this month + ~45 days).
--
-- The experience column on the panel now reads the raw satisfaction_score/
-- sentiment (handled in the component) so it matches the satisfaction page;
-- health_score/health_label are kept in the return for backward compatibility
-- but no longer drive the panel.
...
comment on function public.get_ceo_client_insights(uuid, date, integer) is
  'CEO dashboard action list: clients needing attention for the selected month (open AI risk, overdue installments, low satisfaction, or renewal due soon), ordered by severity. Experience is read from satisfaction_score/sentiment to match the رضا العملاء page.';
