-- 0190: list_tasks_bundle also returns `stage_counts` — a {stage: count} map
-- over the FULL filtered set (not just the paged window). The kanban column
-- headers (جديدة / قيد التنفيذ / ...) use it to show the true per-stage total
-- instead of just the count of currently-loaded cards (120 of N). Rebuilds
-- 0131's body verbatim with two additions: `t.stage` in the `filtered` CTE and
-- a `stage_counts` key in the result object.
...
begin
  ...
  select jsonb_build_object(
    ...
    'stage_counts', coalesce(
      (
        select jsonb_object_agg(g.stage, g.cnt)
        from (
