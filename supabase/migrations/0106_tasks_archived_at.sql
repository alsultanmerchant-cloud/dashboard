-- 0106_tasks_archived_at.sql
--
-- Sky Light §5.2 follow-up: mirror Odoo's project.task.active boolean. In
-- Odoo, archiving a task sets active=false; default views filter it out,
-- the "Archived" search-bar pill flips that to show only archived rows.
--
-- We add a nullable timestamp instead of a bool so we get both the flag
-- and a free audit field ("when was this archived").
--
-- Default tasks query behaviour after this migration: archived rows are
-- hidden unless the caller passes p_archived=true to list_tasks_bundle.

alter table public.tasks
  add column if not exists archived_at timestamptz;

create index if not exists tasks_archived_at_idx
  on public.tasks(organization_id, archived_at)
  where archived_at is not null;

-- Backfill: nothing to do right now — the Odoo importer will set this on
-- the next sync (next code change). Local-created tasks are always active.

-- Extend list_tasks_bundle with two new boolean params:
--   p_archived       — when true, return ONLY archived tasks
--                     (mirrors Odoo's "Archived" filter)
--   p_include_archived — when true, include archived AND active. Use this
--                       for admin / audit views. Defaults to false so the
--                       normal task page hides archived rows by default.
--
-- Postgres won't change the parameter list on `create or replace`, so we
-- drop the existing signature first. Body is the 0105 body with two new
-- where-clauses.
...
begin
  ...
  with filtered as (
    ...
    where t.organization_id = p_org_id
      -- Archived behaviour: hide by default, show ONLY archived when
      -- p_archived, show BOTH when p_include_archived.
      and (
        p_include_archived
