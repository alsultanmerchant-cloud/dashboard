-- 0120_project_sequence_order.sql
--
-- Sky Light feedback — the dashboard project list did not open in the same
-- order as Odoo's kanban. Odoo's `project.project` model orders by
-- `sequence, name, id`; the dashboard RPC ordered purely by `external_id desc`.
--
-- Fix, two parts:
--   1. Add `projects.sequence` (Odoo's manual kanban-sort field; default 10,
--      matching Odoo's own default). The Odoo importer now mirrors it.
--   2. Re-order `list_projects_page_bundle` by `sequence asc, name, external_id`
--      so page 1 matches the Odoo kanban. `name` uses the C collation to
--      reproduce Odoo's byte-order tiebreak (uppercase before lowercase,
--      leading space first) within a shared sequence value.
--
-- Function body is verbatim from 0119 (which carries `external_source` from
-- 0118 and the archived-task-count fix) except: `filtered` now exposes
-- `name`/`sequence`, `page_rows` carries `sequence`, and the two ORDER BY
-- clauses (`page_ids` + final `jsonb_agg`) use the new sort key.
...
begin
  ...
  page_ids as (
    select id
    from filtered
    -- Mirror Odoo `project.project._order` = `sequence, name, id`.
