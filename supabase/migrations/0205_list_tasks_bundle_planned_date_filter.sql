-- 0205: make `planned_date` filterable in list_tasks_bundle's p_date_filters.
-- The whole app's real deadline lives in tasks.planned_date (due_date is empty
-- org-wide), but the date-filter whitelist only accepted due_date /
-- actual_done_date / stage_entered_at / created_at. This adds a planned_date
-- sub-block mirroring the due_date one so "due this week" (and any planned_date
-- window from the search bar) filters correctly. Signature unchanged →
-- create-or-replace, no drop needed. Purely additive: the new block only
-- engages when a caller passes a {field:'planned_date'} entry.
