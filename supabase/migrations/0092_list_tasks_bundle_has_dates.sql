-- 0092_list_tasks_bundle_has_dates.sql
--
-- Sky Light feedback #18 (filter parity): adds `p_has_start_date` and
-- `p_has_end_date` boolean filters to list_tasks_bundle so the smart-search
-- bar can offer Odoo's "Has Start Date" / "Has End Date" presets. In our
-- schema:
--   • start date = tasks.planned_date
--   • end date   = tasks.due_date
--
-- Postgres won't let `create or replace function` change the parameter list,
-- so we drop the existing signature first, then recreate with the two
-- additional bool params (defaulting false so old callers keep working).
-- The function body is otherwise verbatim from 0090.
