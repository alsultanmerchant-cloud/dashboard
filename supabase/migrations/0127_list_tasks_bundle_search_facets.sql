-- 0127_list_tasks_bundle_search_facets.sql
--
-- Rwasem-parity faceted task search.
--
-- The Rwasem (Odoo) search box turns typed text into a field-scoped facet
-- menu — "Search Tasks for X", "Search Tags for X", "Search Assignees for X",
-- "Search Project for X", "Search Stage for X". Each pick becomes a removable
-- facet chip. Combination rules (mirrored here):
--   • facets on DIFFERENT fields  → AND
--   • facets on the SAME field    → OR
--
-- This migration recreates `list_tasks_bundle` with a new trailing parameter
-- `p_search_facets jsonb` — an array of {"field": "...", "value": "..."} —
-- and applies the AND/OR clause. All other parameters and the row payload are
-- verbatim from 0118 (the latest applied definition). The free-text `p_search`
-- parameter is kept untouched so saved filters that carry `q` still work.
--
-- Postgres can't change the parameter list on `create or replace`, so we drop
-- the 0118 signature first.
...
begin
  ...
  with filtered as (
    ...
    where t.organization_id = p_org_id
      ...
      -- AND across distinct fields; OR within the same field. Each branch is
      -- "no facet of this field is present  OR  some facet of this field
      -- matches", so absent fields don't constrain the result.
      and (
