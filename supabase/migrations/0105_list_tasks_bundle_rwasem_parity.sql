-- 0105_list_tasks_bundle_rwasem_parity.sql
--
-- Rwasem filter-parity additions to list_tasks_bundle:
--   • p_unassigned       — tasks with zero task_assignees rows
--   • p_over_timesheets  — logged hours × 60 > allocated_time_minutes
--                           (Rwasem "Timesheets >100%")
--   • p_near_timesheets  — logged hours × 60 ≥ 0.8 × allocated_time_minutes
--                          AND logged hours × 60 < allocated_time_minutes
--                          (Rwasem "Timesheets 80%")
--
-- Row output additions for client-side group-by:
--   • tags — array of {id,name,color} resolved through tasks.project_id
--            → project_tag_assignments → project_tags
--   * (tag grouping is on the project's tags, mirroring Rwasem's behaviour;
--     tasks themselves don't carry tags directly in this schema.)
--
-- Postgres won't change the parameter list on `create or replace`, so we
-- drop the existing signature first. Body is otherwise verbatim from 0092.
