-- 0217 Performance snapshots — add ACTIONS-per-period (Rwasem activity trend)
-- =====================================================================
-- The "كيف يتطوّر نشاط X" modal on نبض الفريق needs an activity-over-time
-- series: how many ACTIONS the person took in Rwasem (Odoo) each period.
-- An action = a stage transition (any stage) OR a Log Note (task_comments),
-- attributed to the agent assignee. (0216 stored completed/on-time/dwell but
-- not actions.) Integrated into the keys union so a period with ONLY actions
-- (no SLA interval, no completion) still produces a row — no gaps in the trend.
-- =====================================================================
...
declare
  ...
  g record;
begin
  -- Base accountable intervals (on-time / dwell / rework) — unchanged from 0216.
  ...
  -- NEW: actions = stage transitions (any stage) + Log Notes, per agent per day.
  create temp table _act on commit drop as
  select org, emp, d, count(*) cnt from (
    select ta.organization_id org, ta.employee_id emp, h.entered_at::date d
      from public.task_assignees ta
      join public.task_stage_history h on h.task_id = ta.task_id
     where ta.role_type = 'agent' and ta.employee_id is not null
    union all
    select ta.organization_id, ta.employee_id, c.created_at::date
      from public.task_assignees ta
      join public.task_comments c on c.task_id = ta.task_id
     where ta.role_type = 'agent' and ta.employee_id is not null
  ) x
  group by org, emp, d;
