-- Single-roundtrip RPC for the /tasks page (both list and kanban views).
-- Replaces buildTaskQuery() in src/lib/data/tasks.ts which currently makes up
-- to 3 PostgREST round-trips before the main fetch (project search + FTS +
-- main query) and then post-filters `assignedToEmployeeId` in JS.
--
-- Returns: jsonb array of task rows, each shaped like the previous PostgREST
-- response (so the JS mapper / row types don't need to change). Wraps in
-- jsonb_build_object so future additions like {rows, total} are non-breaking.
--
-- Filter contract mirrors TaskFilters in tasks.ts — keep these in sync.
...
create or replace function public.list_tasks_bundle(
  ...
  -- jsonb array of {field, from, to}; same field ORs, different fields AND.
...
begin
  if v_search is not null then
    -- websearch_to_tsquery is forgiving for free-text user input; the
    -- arabic config matches migration 0061's GIN index so this hits the index.
  ...
  with filtered as (
    ...
    where t.organization_id = p_org_id
      ...
      and (
        ...
        -- 3+ buckets selected = no constraint (matches TS: `length < 3`).
      ...
      -- Date filters: jsonb array of {field, from, to}. Same field ORs together,
      -- different fields AND. Only the four DATE_FIELDS are honored; unknown
      -- field names are ignored.
      and (
        p_date_filters is null or jsonb_array_length(p_date_filters) = 0
        or (
          (
            -- due_date constraint: ignored if no entries on this field.
