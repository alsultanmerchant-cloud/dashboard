-- 0242: make the /tasks free-text search (q param) also match task_code.
--
-- The search vector `search_tsv` is built from title + description only (0061),
-- so `q=PRJ-01826-022` runs websearch_to_tsquery over a vector that never
-- contains the code → zero rows → the empty "no tasks" page. Several surfaces
-- deep-link to a task by CODE via `/tasks?q=<code>` (notably the /satisfaction
-- accountability cards, which only carry task codes from the frozen roster and
-- can't always resolve a task uuid). Add a task_code ILIKE branch to the search
-- predicate so those links resolve to the one matching task.
--
-- Recreated from 0217 verbatim (BOTH the 0215 critical-delay predicate AND the
-- 0212 stage-facet enum match preserved) with a single added OR branch in the
-- v_search block. Idempotent create-or-replace; signature unchanged.
