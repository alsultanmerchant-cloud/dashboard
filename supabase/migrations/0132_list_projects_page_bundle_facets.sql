-- 0132_list_projects_page_bundle_facets.sql
--
-- Rwasem-parity faceted project search.
--
-- Mirrors migration 0127 (tasks side). Typed text in the projects search bar
-- becomes a field-scoped facet chip (Search Project for X, Search Code for X,
-- Search Client for X, Search Manager for X, Search Tags for X). Combination
-- rules:
--   • facets on DIFFERENT fields  → AND
--   • facets on the SAME field    → OR
--
-- Recreate `list_projects_page_bundle` with a trailing `p_search_facets jsonb`
-- parameter. All other parameters and the row payload are verbatim from 0130
-- (the latest applied definition). The free-text `p_search` parameter is
-- preserved so saved searches with `?q=` still work.
...
begin
  with filtered as (
    ...
    where p.organization_id = p_org_id
      ...
      -- AND across distinct fields; OR within the same field. Each branch
      -- evaluates to "no facet of this field is present  OR  some facet of
      -- this field matches", so absent fields don't constrain the result.
      and (
