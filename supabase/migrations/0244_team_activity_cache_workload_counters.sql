-- 0244 — Team activity page: move workload counters off the request path
-- =========================================================================
-- /team-activity already reads most movement metrics from team_activity_cache,
-- but still recalculated three workload counters live on every page load:
--   * owned_open
--   * pending_late (current-stage SLA breach)
--   * active_projects
--
-- The pending_late aggregate calls business_minutes_between() per open task,
-- which makes cold page loads spike. Keep the exact same semantics, but compute
-- these counters during the 10-minute refresh job instead of in the page query.
-- =========================================================================

alter table public.team_activity_cache
  add column if not exists owned_open int not null default 0,
  add column if not exists pending_late int not null default 0,
  add column if not exists active_projects int not null default 0;
