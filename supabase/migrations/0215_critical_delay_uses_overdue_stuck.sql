-- 0215: fix the `critical` task filter ("تأخيرات حرجة") drill-down.
--
-- The dashboard executive-scores card counts criticalOverdue as OPEN tasks that
-- are is_overdue AND stuck in their current stage > 14 days (executive-scores.ts).
-- But the matching task-list filter (p_critical_delay) used `delay_days > 3` —
-- and delay_days is unpopulated org-wide (the Odoo sync never fills it), so the
-- predicate matched ZERO rows and the drill-down opened an empty page.
--
-- Realign the filter to the card's definition: overdue + stalled in stage > 14d,
-- excluding done. Now the "تأخيرات حرجة" pill and the page it opens describe the
-- exact same task set. Same class of fix as 0212 (stage drill-down → empty page).
--
-- Signature unchanged → create-or-replace. ONLY line changed vs 0205 is the
-- p_critical_delay predicate; the rest is verbatim.
