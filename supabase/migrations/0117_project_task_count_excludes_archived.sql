-- 0117_project_task_count_excludes_archived.sql
--
-- Sky Light feedback #4 — the task-count badge on project cards counted
-- archived tasks. Example: PRJ-01587 showed "177 مهام" while the project
-- actually has 28 active tasks.
--
-- Root cause: the `tc` lateral inside `list_projects_page_bundle` (last
-- redefined in 0114) counts every row in `tasks` for the project with no
-- regard for `archived_at`. Stock Odoo's `project.task_count` is computed
-- over `project.task` records filtered by `active = True` — archived tasks
-- are excluded.
--
-- Fix: add `and t.archived_at is null` to the three counters in the `tc`
-- lateral so `task_count`, `open_task_count` and `closed_task_count` mirror
-- Odoo's active-only semantics. Body is verbatim from 0114 except for that
-- one line; the signature is unchanged so we can use create-or-replace.
...
begin
  ...
  page_rows as (
    ...
    left join lateral (
      -- Sky Light #4: count ACTIVE tasks only. `archived_at is null` mirrors
      -- stock Odoo's `project.task_count` which is computed over rows where
      -- `active = True`. Archived tasks were inflating the card badge.
      select
        count(*) as task_count,
