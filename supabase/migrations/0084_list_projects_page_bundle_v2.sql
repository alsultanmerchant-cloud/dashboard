-- Tighten the /projects page RPC. Same signature, same JSON shape — only the
-- internals change. Two wins over migration 0083:
--
-- 1) `language plpgsql` instead of `language sql` so Postgres caches the
--    prepared plan after a handful of invocations (custom→generic promotion).
--    Drops planning cost (was ~25ms in EXPLAIN, larger than execution).
--
-- 2) Replace the dependency on the `project_task_counts` view (which seq-scans
--    every task in the org and aggregates per-project, only to discard all but
--    the 25 paged rows) with a LEFT JOIN LATERAL that counts only the page's
--    projects via `idx_tasks_project`. Drops the per-call hot spot from ~8.7ms
--    to ~2ms and cuts buffer hits by ~280 pages.
--
-- (Tried collapsing the filtered count via `count(*) over ()` on page_ids — but
-- when the page offset is past the end the page is empty and we lose `total`.
-- Kept the explicit `count(*) from filtered` for correctness; cost is ~0.7ms.)
