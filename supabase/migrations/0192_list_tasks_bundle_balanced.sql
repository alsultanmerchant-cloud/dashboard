-- 0192: balanced per-column kanban loading. Adds `p_partition_by` +
-- `p_partition_limit`: when set, the paged window returns the top-N newest tasks
-- PER BUCKET (e.g. per stage / priority / project) instead of the global
-- newest-N. This keeps every kanban column populated up front instead of the
-- newest tasks all piling into one or two buckets while the rest show a single
-- card. `total_count` and `group_rows` still span the FULL filtered set, so
-- header totals stay true. Only SQL-partitionable dimensions are supported
-- (stage / priority / status / progress / service / project / customer); the
-- caller passes null for multi-valued (assignee / tags) or client-tz date
-- group-bys, which fall back to a flat (larger) load. Adding params changes the
-- signature, so the prior overload is dropped first to avoid PostgREST PGRST203
-- ambiguity.
--
-- (From 0191) list_tasks_bundle also returns `group_rows` — a lightweight array
-- of grouping keys for EVERY task in the filtered set, which the kanban buckets
-- client-side (same `bucketTasksBy` as the cards) for true per-column totals.

drop function if exists public.list_tasks_bundle(
  uuid, integer, integer, text[], text[], text[], uuid, boolean, boolean,
  boolean, boolean, boolean, text[], boolean, uuid, uuid, text, jsonb, boolean,
  boolean, boolean, boolean, boolean, boolean, boolean, boolean, jsonb, uuid[]
);
