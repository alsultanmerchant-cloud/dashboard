-- 0114_list_projects_page_bundle_id_whitelist.sql
--
-- Adds an optional `p_id_whitelist uuid[]` parameter to
-- `list_projects_page_bundle` so the Node layer can pre-filter project IDs
-- via arbitrary PostgREST clauses (the Odoo-style "Add Custom Filter"
-- builder) and then call the bundle RPC with the whitelisted set.
--
-- When `p_id_whitelist` is null (the default), behaviour is identical to v3
-- — all existing callers continue to work unmodified.
-- When non-null, the `filtered` CTE additionally requires `p.id = any(p_id_whitelist)`.
--
-- Postgres won't let us mutate an existing signature with create-or-replace
-- (a new param shifts the function identity), so we drop v3 first and
-- recreate. Body is verbatim from 0093 except for the added column on the
-- final `and (...)` line and the new param at the head.
